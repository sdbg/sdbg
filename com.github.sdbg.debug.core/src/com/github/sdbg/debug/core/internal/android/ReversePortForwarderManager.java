...
public class ReversePortForwarderManager {
  private static final String APP_NAME = "com.github.sdbg.android.forwarder",
      APP_FILE_NAME = APP_NAME + ".jar", APP_DEVICE_DIR = "/data/local/" + APP_NAME,
  ...
  public IProcess start(ILaunch launch, IDeviceChooser deviceChooser, int deviceCommandPort,
    ...
    final IDeviceInfo deviceInfo = deviceChooser.chooseDevice(manager.getDevices());
    if (deviceInfo == null) {
  ...
  private Process prepareDeviceExecutableProcess(ADBManager manager, String deviceId,
    ...
    return manager.asyncShell(deviceId, "dalvikvm -cp " + APP_FILE_NAME + " " + MAIN_CLASS_NAME
        + deviceCommandPort + " " + devicePortsStr);
  }

  private void pushDeviceExecutable(ADBManager manager, String deviceId) throws CoreException {
    manager.shell(deviceId, "mkdir " + APP_DEVICE_DIR);
    ...
    try {
      ...
      try {
        ...
        manager.push(deviceId, tempFile, APP_DEVICE_DIR + "/" + APP_FILE_NAME);
      } finally {
        tempFile.delete();
      }
    } catch (IOException e) {
